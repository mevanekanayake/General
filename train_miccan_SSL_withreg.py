...
def f(image2, mask):
    k_hat = ft(image2.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
    ku_hat = mask * k_hat
    return ku_hat
    ...
    for _ in range(args.ne):
        ...
        with tqdm(train_loader, unit="batch") as train_epoch:
            ...
            for batch in train_epoch:
                ...
                optimizer.zero_grad()
                _, x_hat2 = model(xu, yu, msk)

                train_loss = loss(x_hat2, yu, msk)
        ...
        with torch.no_grad():
            ...
            with tqdm(val_loader, unit="batch") as val_epoch:
                for batch in val_epoch:
                    ...
                    val_loss = loss(x_hat2, yu, msk)

                    # END VALIDATION STEP
                    val_epoch.set_postfix(val_loss=val_loss.detach().item())
                    m.end_val_step(fname, slice_num, sequence, batch.image_zf, x_hat.to('cpu'), batch.target, val_loss.to('cpu'))
            ...
            if m.best_epoch or (m.epoch_count % args.pf == 0):
                with tqdm(viz_loader, unit="batch") as viz_epoch:
                    for batch in viz_epoch:
                        ...
                        x_hat, _ = model(xu, yu, msk)

                        # END VISUALIZATION STEP
                        m.visualize(fname, slice_num, sequence, batch.image_zf, x_hat.to('cpu'), batch.target)
