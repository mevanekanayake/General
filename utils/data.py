    ...
    def __init__(self, root, train, nv=0, transform=None):
        ...
        if nv != 0:
            ...
            for seq_type in self.seq_types:
                ...
                self.selected_examples.append(random.choice([item[0][0] for item in self.library[self.key][seq_type][:vols_per_seq]])) if not train else None
                # collect the file name of the first slice from a randomly selected volume from each sequence out of the sampled volumes
        else:
            ...
            for seq_type in self.seq_types:
                ...
                # collect the file name of the first slice from a randomly selected volume from each sequence out of all volumes
