...
class RunManager:
    ...
    def __init__(self, train_loader, val_loader, experiments_path, local_device, ckpt):
    ...
    def end_val_step(self, val_logs, size_of_val_batch):
        ...
        for i, fname in enumerate(val_logs["fname"]):
            ...
            self.target_norms[fname][slice_num] = torch.tensor(metrics.mse(target, np.zeros_like(target))).view(1)
            self.ssim_vals[fname][slice_num] = torch.tensor(
                metrics.ssim(target[None, ...], output[None, ...], maxval=maxval)).view(1)
